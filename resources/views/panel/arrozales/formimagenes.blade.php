...
@section('title')
    {{!isset($arrozalesimagen->id) ? trans_choice('sentences.imagen-agregar', 1) : trans_choice('sentences.imagen-editar', 1) }}
@endsection
@section('content')
    <div class="content">
        <div class="row">
            <div class="col-md-12">
                <form id="RegisterValidation"
                    action="{{$arrozalesimagen->id ? route('arrozalesimagenes.update', $arrozalesimagen->id): route('arrozalesimagenes.store') }}"
                    method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    @if ($arrozalesimagen->id)
                        {{ method_field('PATCH') }}
                    @endif
                    <div class="card card-nav-tabs">
                        <div class="card-header card-header-tabs nav-pills-primary">
                            <h4 class="card-title m-2">
                                {!! $arrozalesimagen->id ? trans_choice('sentences.imagen-agregar', 1)  : trans_choice('sentences.imagen-editar', 1) !!}
                            </h4>
                        </div>
                        <div class="card-body">
                                ...
                                <div class="form-group">
                                    <div class="fileinput fileinput-new text-center" data-provides="fileinput">
                                        <div class="fileinput-new thumbnail">
                                            <img src="{{$arrozalesimagen->imagen ? url('uploads/'.$arrozalesimagen->imagen):url('assets_template/img/image_placeholder.jpg')}}"
                                                 alt="Imagen">
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <span class="fileinput-exists">{{ trans_choice('sentences.cambiar', 1) }}</span>
                                                <input type="file" name="imagen" id="imagen" {{ $arrozalesimagen->id ? '' : 'required="true"' }} accept="image/*"/></span>
                        ...
                        <div class="card-footer text-right">
                            <button type="submit"
                                    class="btn btn-primary">{{$arrozalesimagen->exists ? trans_choice('sentences.guardar', 1)  : trans_choice('sentences.agregar', 1)  }}</button>
                        </div>
                    </div>
                </form>
            </div>
