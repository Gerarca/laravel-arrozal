...
@section('title')
    {{!isset($diacampoimagen->id) ? trans_choice('sentences.imagen-agregar', 1) : trans_choice('sentences.imagen-editar', 1) }}
@endsection
@section('content')
    <div class="content">
        <div class="row">
            <div class="col-md-12">
                <form id="RegisterValidation"
                    action="{{$diacampoimagen->id ? route('diacampoimagenes.update', $diacampoimagen->id): route('diacampoimagenes.store') }}"
                    method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    @if ($diacampoimagen->id)
                        {{ method_field('PATCH') }}
                    @endif
                    <div class="card card-nav-tabs">
                        <div class="card-header card-header-tabs nav-pills-primary">
                            <h4 class="card-title m-2">
                                {!! $diacampoimagen->id ? trans_choice('sentences.imagen-editar', 1) : trans_choice('sentences.imagen-agregar', 1) !!}
                            </h4>
                        </div>
                        <div class="card-body">
                                ...
                                <div class="form-group">
                                    <div class="fileinput fileinput-new text-center" data-provides="fileinput">
                                        <div class="fileinput-new thumbnail">
                                            <img src="{{$diacampoimagen->imagen ? url('uploads/'.$diacampoimagen->imagen):url('assets_template/img/image_placeholder.jpg')}}"
                                                 alt="Imagen">
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <span class="fileinput-exists">{{ trans_choice('sentences.cambiar', 1) }}</span>
                                                <input type="file" name="imagen" id="imagen" {{ $diacampoimagen->id ? '' : 'required="true"' }} accept="image/*"/></span>
                            ...
                            <div class="form-group has-label">
                            <div class="category form-category">* {{ trans_choice('sentences.campos-requridos', 1) }}</div>
                        </div>
                        <div class="card-footer text-right">
                            <button type="submit"
                                    class="btn btn-primary">{{$diacampoimagen->exists ? trans_choice('sentences.guardar', 1): trans_choice('sentences.agregar', 1) }}</button>
                        </div>
                    </div>
                </form>
            </div>
