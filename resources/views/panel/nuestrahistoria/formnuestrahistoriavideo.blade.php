...
@section('title')
    {{!isset($nuestrahistoriavideo->id) ? trans_choice('sentences.agregar', 1) .' Video' :  trans_choice('sentences.editar', 1) .' Video' }}
@endsection
@section('content')
    <div class="content">
        <div class="row">
            <div class="col-md-12">
                <form id="RegisterValidation"
                    action="{{$nuestrahistoriavideo->id ? route('nuestrahistoriavideo.update', $nuestrahistoriavideo->id): route('nuestrahistoriavideo.store') }}"
                    method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    @if ($nuestrahistoriavideo->id)
                        {{ method_field('PATCH') }}
                    @endif
                    <div class="card card-nav-tabs">
                        <div class="card-header card-header-tabs nav-pills-primary">
                            <h4 class="card-title m-2">
                                {!! !$nuestrahistoriavideo->id ? trans_choice('sentences.agregar', 1) .' Video' : trans_choice('sentences.editar', 1) .' Video' !!}
                            </h4>
                        </div>
                        <div class="card-body">
                        ...
                        <div class="card-footer text-right">
                            <button type="submit"
                                    class="btn btn-primary">{{$nuestrahistoriavideo->exists ? trans_choice('sentences.guardar', 1) : trans_choice('sentences.agregar', 1) }}</button>
                        </div>
                    </div>
                </form>
            </div>
